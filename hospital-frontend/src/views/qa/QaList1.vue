<template>
  <div>
    ...
    <el-dialog title="咨询记录" :visible.sync="modifyFormVisible">
      <div>
        <p style="white-space: pre-wrap;">{{ modifyForm.qaHistory ? modifyForm.qaHistory.replaceAll(userName, '我') : modifyForm.qaHistory}}</p>
      </div>
      ...
      request
          ...
          .then((res) => {
            ...
            this.qaData.forEach((item, index) => {
              this.qaData[index].qaHistory = this.qaData[index].qaHistory.replaceAll(this.userName, '我');
            });
